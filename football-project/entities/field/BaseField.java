...
public  class BaseField implements Field{
    ...
    public String getInfo() {
        ...
        String playerReport = players.isEmpty() ? "none"
                : players.stream().map(Player::getName).collect(Collectors.joining(" "));

        return String.format("%s (%s):%n" + "Player: %s%n" + "Supplement: %d%n" + "Energy: %d",name,getClass().getSimpleName(),playerReport,supplements.size(),sumEnergy());
    }
